...
func testPixi(t *testing.T, context spec.G, it spec.S) {
	var (
	...
	context("when building a pixi app", func() {
		var (
			image     occam.Image
		...
		it("creates a working OCI image with a start command", func() {
			var err error
			source, err = occam.Source(filepath.Join("testdata", "pixi"))
			Expect(err).NotTo(HaveOccurred())
			...
			Expect(logs).To(ContainLines(ContainSubstring("Buildpack to install python package managers")))
			Expect(logs).To(ContainLines(ContainSubstring("Buildpack for all python package managers")))
		...
		context("when using CA certificates", func() {
			...
			it("builds a working OCI image with a start command and uses a client-side CA cert for requests", func() {
				...
				image, logs, err = pack.WithNoColor().Build.
					WithBuildpacks(pythonBuildpack).
					WithPullPolicy("never").
					Execute(name, filepath.Join(source, "pixi"))
				Expect(err).NotTo(HaveOccurred())
